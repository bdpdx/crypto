...
class aes : public aes_context {

public:

	// in_key_bytes must be one of { 16, 24, 32 }
	aes() { }
	aes( const void *in_key, __u32 in_key_bytes );
   ~aes();

	void init( const void *in_key, __u32 in_length );
	
	// caller must delete[] out_data.  out_data may be padded with pad bytes
	// to allow for easier decryption of c-strings.  pad bytes will be appended
	// to decrypted data and zeroed.  The count of pad bytes will NOT be
	// included in out_length.
	void decrypt( const void *in_data, __u32 in_length, __u8 *&out_data, __u32 &out_length, __u32 in_out_data_pad_bytes = 0 );
	// caller must delete[] out_data
	void encrypt( const void *in_data, __u32 in_length, __u8 *&out_data, __u32 &out_length );

};

#endif // ENABLE_AES_CPP
#endif // __cplusplus
